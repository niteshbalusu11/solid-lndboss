...
import { ambossUrl } from 'src/utils/constants';
import axios from 'axios';
import { checkAmbossHealthSetting } from 'src/settings';
import { getSavedNodes } from 'src/lnd';
...
const ambossHealthCheck = async ({
  logger,
}): Promise<{ postToAmboss: any }> => {
  return auto<Tasks>({
    // Check arguments
    validate: (cbk: any) => {
      if (!logger) {
    ...
    getSavedNodes: [
      ...
      ({}, cbk: any) => {
        const retryGetSavedNodes = async () => {
          ...
          const filteredNodes = nodes.nodes.filter(
            (node) => !!node.lnd && !!node.is_online,
          );
          ...
          each(offlineNodes, (node) => {
            logger.log({
              type: 'warn',
              message: `OfflineNode ${node.node_name}`,
            });
          });
        ...
        retry(
          ...
          (err, res) => {
            if (!!err) {
              return cbk([
                500,
                'UnexpectedErrorGettingSavedNodesToPostAmbossHealthCheck',
                err,
              ]);
            }
              ...
              each(nodes, async (node) => {
                try {
                  const isHealthCheckEnabled =
                    (await checkAmbossHealthSetting()) ||
                    process.env.AMBOSS_HEALTH_CHECK === 'true';
