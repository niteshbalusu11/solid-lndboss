import { encryptString, httpLogger } from 'src/utils/global_functions';
import { existsSync, mkdir, writeFile } from 'fs';

import { auto } from 'async';
import { encryptionKey } from 'src/utils/constants';
...
const putSavedCredentials = async (
  args: Args,
  ...
  const result = await auto<Tasks>({
    ...
    validate: (cbk: any) => {
      if (!args.auth_type) {
        return cbk(
          httpLogger({ error: [400, 'ExpectedAuthTypeToPutSavedCredentials'] }),
        );
      }

      if (args.auth_type === 'credentials' && !args.macaroon) {
        return cbk(
          httpLogger({ error: [400, 'ExpectedMacaroonToPutSavedCredentials'] }),
        );
      }
      ...
      if (!args.node) {
        return cbk(
          httpLogger({
            error: [400, 'ExpectedSavedNodeNameToPutSavedCredentials'],
          }),
        );
      }
    ...
    isValidPath: [
      ...
      ({}, cbk: any) => {
        ...
        if (!existsSync(args.cert_path)) {
          return cbk(
            httpLogger({
              error: [400, 'ExpectedValidTlsCertPathToPutSavedCredentials'],
            }),
          );
        }

        if (!existsSync(args.macaroon_path)) {
          return cbk(
            httpLogger({
              error: [400, 'ExpectedValidMacaroonPathToPutSavedCredentials'],
            }),
          );
        }
      ...
      ({}, cbk: any) => {
        ...
        return writeFile(path, file, (err: any) => {
          if (!!err) {
            return cbk(
              httpLogger({
                error: [503, 'UnexpectedErrorWritingConfigFile', { err }],
              }),
            );
          }
      ...
      ({}, cbk: any) => {
        ...
        return writeFile(path, file, (err: any) => {
          if (!!err) {
            return cbk(
              httpLogger({
                error: [503, 'UnexpectedErrorWritingSavedCredentials', { err }],
              }),
            );
          }
    ...
    encryptMacaroon: [
      ...
      ({}, cbk: any) => {
        ...
        if (!!error) {
          return cbk(
            httpLogger({
              error: [503, 'UnexpectedErrorEncryptingMacaroon', { error }],
            }),
          );
        }
    ...
    finalCredentials: [
      ...
      ({ encryptMacaroon }, cbk: any) => {
        ...
        if (
          !encryptMacaroon ||
          !encryptMacaroon.encryptedMacaroon ||
          !encryptMacaroon.iv
        ) {
          return cbk(null, {
            cert: args.cert,
