...
const lndCredentials = async (args: Args): Promise<Return> => {
  const result = await auto<Tasks>({
          ...
          try {
            ...
            return cbk(
              httpLogger({
                error: [
                  400,
                  'ExpectedValidConfigFileToGetCredentials',
                  { err },
                ],
              }),
            );
    ...
    credentials: [
      ...
      'getSocket',
      (
        { forNode, getNodeCredentials, getCert, getMacaroon, getSocket },
        cbk,
      ) => {
  ...
  return {
    ...
    macaroon: result.credentials.macaroon,
    socket: result.credentials.socket,
  };
};
