...
import { channelBalance, walletInfo } from 'src/commands/grpc_utils/grpc_utils';
import { getPendingDto, grpcDto } from 'src/shared/commands.dto';

import { Injectable } from '@nestjs/common';
import { LndService } from '../lnd/lnd.service';
import getPeers from 'src/commands/grpc_utils/get_peers';
import { getPending } from 'src/commands/grpc_utils';
...
export class GrpcService {
  ...
  async getPeersAllNodes(): Promise<any> {
    ...
    const result = await map(
      onlineNodes,
      async (node: { lnd: AuthenticatedLnd; node_name: any }) => {
