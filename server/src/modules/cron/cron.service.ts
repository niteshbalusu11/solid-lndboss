...
import autoRebalanceCommand from 'src/commands/rebalance/auto_rebalance_command';
import { httpLogger } from 'src/utils/global_functions';
import { rebalanceScheduleDto } from 'src/shared/commands.dto';
...
export class CronService {
  ...
  async createRebalanceCron({ args, id }: RebalanceCronArgs) {
    this.logger.log({
      message: `adding cron schedule ${stringify(args)}`,
      type: 'info',
    });
  ...
  async createAutoFeesCron({ args, schedule }: AutoFeeCronArgs) {
    this.logger.log({
      message: `adding fee cron schedule ${schedule}:\n${stringify(args)}`,
      type: 'info',
    });
  ...
  async deleteCron({ name }: { name: string }) {
    ...
    this.logger.log({
      message: `deleting cron schedule ${name}`,
      type: 'warn',
    });
  }
}
