...
import { autoFeesCronSchedule, removeStyling } from 'src/utils/constants';
import { feesDto, feesStrategiesDto } from 'src/shared/commands.dto';

import { CronService } from '../cron/cron.service';
import { LndService } from '../lnd/lnd.service';
import { feesCommand } from 'src/commands';
import { httpLogger } from 'src/utils/global_functions';
import readFeesFile from 'src/commands/fees/read_fees_file';
import saveStrategies from 'src/commands/fees/save_strategies';
import scheduledFeesCommand from 'src/commands/fees/scheduled_fees_command';
import validateStrategies from 'src/commands/fees/validate_strategies';
...
export class FeesService implements OnModuleInit {
  ...
  async createAutoFeesCron() {
    try {
      ...
      data.configs.forEach((config) => {
        this.cronService.createAutoFeesCron({
          args: config,
          schedule: autoFeesCronSchedule,
        });
      });
    } catch (error) {
      httpLogger({ error });
  ...
  async runAutoFees(args: feesStrategiesDto) {
    try {
      const lnd = await LndService.authenticatedLnd({
        node: args.configs.node,
      });
  ...
  async save(args: feesStrategiesDto) {
    try {
      ...
      const getJob = await this.cronService.getCronJob({
        name: args.configs.message_id,
      });
