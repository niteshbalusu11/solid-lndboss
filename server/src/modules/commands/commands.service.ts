...
import * as dto from 'src/shared/commands.dto';

import { Logger, createLogger, format, transports } from 'winston';
import { getSettingsFile, writeSettingsFile } from 'src/settings';

import { Injectable } from '@nestjs/common';
import { LndService } from '../lnd/lnd.service';
import { SocketGateway } from '../socket/socket.gateway';
import { httpLogger } from 'src/utils/global_functions';
import { removeStyling } from 'src/utils/constants';
import validateOpen from 'src/commands/open/validate_open';
...
export class CommandsService {
  ...
  async certValidityDaysCommand(
    args: dto.certValidityDaysDto,
  ): Promise<{ result: any }> {
    const { result } = await commands.certValidityDaysCommand({
      below: args.below,
      node: args.node,
    });
  ...
  async chartChainFeesCommand(
    args: dto.chartChainFeesDto,
  ): Promise<{ result: any }> {
    const lnds = await LndService.getLnds({ nodes: args.nodes });
    const { result } = await commands.chartChainFeesCommand({
      args,
      lnd: lnds,
    });
  ...
  async chartFeesEarnedCommand(
    args: dto.chartFeesEarnedDto,
  ): Promise<{ result: any }> {
    const lnds = await LndService.getLnds({ nodes: args.nodes });

    const { result } = await commands.chartFeesEarnedCommand({
      args,
      lnd: lnds,
    });
  ...
  async chartFeesPaidCommand(
    args: dto.chartFeesPaidDto,
  ): Promise<{ result: any }> {
    const lnds = await LndService.getLnds({ nodes: args.nodes });
  ...
  async chartPaymentsReceivedCommand(
    args: dto.chartPaymentsReceivedDto,
  ): Promise<{ result: any }> {
    const lnds = await LndService.getLnds({ nodes: args.nodes });

    const { result } = await commands.chartPaymentsReceivedCommand({
      args,
      lnd: lnds,
    });
  ...
  async cleanFailedPaymentsCommand(
    args: dto.cleanFailedPaymentsDto,
  ): Promise<{ result: any }> {
    try {
      const lnd = await LndService.authenticatedLnd({ node: args.node });

      const logger: Logger = await this.defaultLogger({
        service: 'cleanfailedpayments',
      });

      const { result } = await commands.cleanFailedPaymentsCommand({
        args,
        lnd,
        logger,
      });
  ...
  async createChannelGroupCommand(
    args: dto.createGroupChannelDto,
  ): Promise<{ result: any }> {
    try {
      const logger = await this.logger({
        messageId: args.message_id,
        service: 'create-group-channel',
      });

      const lnd = await LndService.authenticatedLnd({ node: args.node });

      const { result } = await commands.createChannelGroupCommand({
        args,
        lnd,
        logger,
      });
  ...
  async joinChannelGroupCommand(
    args: dto.joinGroupChannelDto,
  ): Promise<{ result: any }> {
    try {
      const logger = await this.logger({
        messageId: args.message_id,
        service: 'join-group-channel',
      });

      const lnd = await LndService.authenticatedLnd({ node: args.node });

      const { result } = await commands.joinChannelGroupCommand({
        args,
        lnd,
        logger,
      });
