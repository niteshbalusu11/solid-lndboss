...
import {
  deleteRebalanceDto,
  rebalanceDto,
  rebalanceScheduleDto,
} from 'src/shared/commands.dto';
...
import manageRebalanceTriggers from 'src/commands/rebalance/manage_rebalance_triggers';
import { rebalanceCommand } from 'src/commands';
...
export class RebalanceService implements OnModuleInit {
    ...
    try {
      ...
      if (!!result && !!result.length) {
        result.forEach((rebalance) =>
          this.cronService.createRebalanceCron({
            args: rebalance.rebalance_data,
            id: rebalance.id,
          }),
        );
      }
  ...
  // Manual rebalance
  async rebalance(
    args: rebalanceDto | rebalanceScheduleDto,
  ): Promise<{ result: any }> {
  ...
  async scheduleRebalance(
    ...
    try {
      ...
      if (!!result.createRebalanceTrigger) {
        this.cronService.createRebalanceCron({
          args,
          id: result.createRebalanceTrigger.id,
        });
      }
