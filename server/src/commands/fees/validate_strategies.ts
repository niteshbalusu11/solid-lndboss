      ...
      validateConfig: [
        ...
        async ({ getTags }) => {
          ...
          return await each(configs.config, async (config) => {
            ...
            if (
              !!config.inactivity.filter((n) => !isNumber(Number(n))).length
            ) {
              throw new Error('ExpectedNumericFeeRateValues');
            }

            if (
              !!config.maxhtlcratios.filter((n) => !isNumber(Number(n))).length
            ) {
            ...
            if (
              !!config.inactivity.filter((n) => !isInteger(Number(n))).length
            ) {
              throw new Error('ExpectedInactivityPeriodsGreaterThanZero');
            }

            if (
              !!config.maxhtlcratios.filter(
                (n) => Number(n) < 0 || Number(n) > 1,
              ).length
            ) {
              throw new Error(
                'ExpectedMaxHtlcRatioLessThanOneAndGreaterThanZero',
              );
            }
            ...
            config.ratios.forEach((n) => {
              ...
              split.forEach((n) => {
                ...
                if (Number(n) > 1 || Number(n) < 0) {
                  throw new Error(
                    'Expected Outbound Capacity Ratio Less Than One And Greater Than Zero In Row',
                  );
                }
              });
              ...
              if (!checkRatio(split)) {
                throw new Error(
                  'Expected First Number In Ratio Lower Than Second Number In Row',
                );
              }
            });
