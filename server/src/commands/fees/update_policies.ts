    ...
    await auto<Tasks>({
      ...
      getForwards: [
        ...
        async () => {
          ...
          const numericInactivity = args.config.inactivity.map((n) =>
            Number(n),
          );
          const maxInactivity = Math.max.apply(null, numericInactivity);

          return await fetchForwards({
            idle_days: maxInactivity,
            lnd: args.lnd,
          });
        },
      ],
      ...
      getPeers: [
        ...
        ({}, cbk: any) => {
          ...
          return map(
            ...
            (query, cbk) => {
              const nodes = args.getIcons.nodes.filter((n) =>
                n.aliases.includes(query),
              );
      ...
      getUpdateInfo: [
        ...
        async ({ getPeers }) => {
          ...
          const combinedArray = args.config.ratios.map((ratio, i) => {
            return {
              ratio,
              basefees: !!args.config.basefees.length
                ? args.config.basefees[i]
                : undefined,
              feerate: !!args.config.feerates.length
                ? Number(args.config.feerates[i])
                : undefined,
              inactivity: !!args.config.inactivity.length
                ? Number(args.config.inactivity[i])
                : undefined,
              maxhtlcratio: !!args.config.maxhtlcratios.length
                ? Number(args.config.maxhtlcratios[i])
                : undefined,
            };
          });
          ...
          const res = await map(
            combinedArray,
            async ({ ratio, basefees, feerate, inactivity, maxhtlcratio }) => {
              const getBetweenChannels = await map(
                peerKeys,
                async (key: string) => {
                  const channels = args.getChannels.channels
                    ...
                    .filter((channel) => {
                      ...
                      const outboundCapacityRatio = div(
                    ...
                    .map((n) => ({
                      ...
                      inactivity: moment()
                        .subtract(inactivity, 'days')
                        .toISOString(),
                      max_htlc_mtokens: !!maxhtlcratio
                        ? maxhtlc(n.capacity, maxhtlcratio)
                        : undefined,
        ...
        async ({ getUpdateInfo, getForwards }) => {
          ...
          const filterInfo = getUpdateInfo.filter((info) => {
            const forward = getForwards.find(
              (n) =>
                (n.incoming_channel === info.channel_id ||
                  n.outgoing_channel === info.channel_id) &&
