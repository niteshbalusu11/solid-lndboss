    ...
    await auto<Tasks>({
          ...
          return map(
            ...
            (key: string, cbk) => {
              return getNode({ lnd: args.lnd, public_key: key }, (err, res) => {
                ...
                const isBig = res.features.find(
                  (n) => n.type === 'large_channels',
                );
      ...
      checkNetworks: [
        ...
        ({ getNetwork, getOpeningNetworks }, cbk: any) => {
          ...
          if (
            !!getOpeningNetworks.find((n) => n.network !== getNetwork.network)
          ) {
