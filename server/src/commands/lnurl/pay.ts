...
const tokensAsMtokens = (tokens: number): string =>
  Math.floor(tokens * 1e3).toString();
...
const pay = async (args: Args): Promise<Tasks> => {
  return auto<Tasks>({
    ...
    validateTerms: [
      ...
      ({ getTerms }, cbk: any) => {
        if (getTerms.min > args.amount) {
          return cbk([
            400,
            'AmountBelowMinimumAcceptedAmountToPayViaLnurl',
            getTerms.min,
          ]);
        }
        ...
        if (getTerms.max < args.amount) {
          return cbk([
            400,
            'AmountAboveMaximumAcceptedAmountToPayViaLnurl',
            getTerms.max,
          ]);
        }
    ...
    getAlias: [
      'getRequest',
      async ({ getRequest }) => {
        return await getNodeAlias({
          id: getRequest.destination,
          lnd: args.lnd,
        });
      },
    ],
