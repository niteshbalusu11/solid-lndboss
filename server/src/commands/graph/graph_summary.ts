...
const isClear = (sockets) =>
  !!sockets.find((n) => !!isIP(n.socket.split(':')[0]));
const isLarge = (features) =>
  !!features.find((n) => n.type === 'large_channels');
...
const graphSummary = async ({ args, lnd }): Promise<{ nodeDetails: any }> => {
  return auto<Tasks>({
    ...
    getKey: [
      'validate',
      async ({}) => await findKey({ lnd, query: args.query }),
    ],
    ...
    getNode: [
      'key',
      async ({ key }) => await getNode({ lnd, public_key: key }),
    ],
