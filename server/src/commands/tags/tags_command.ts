    ...
    const result = await auto<Tasks>({
        ...
        ({}, cbk: any) => {
          return readFile(tagFilePath(), (err, res) => {
            ...
            try {
              ...
              return cbk([
                400,
                'ExpectedValidJsonTagsFileToAdjustTags',
                { err },
              ]);
            }
      ...
      adjustTag: [
        ...
        ({ getTags }, cbk: any) => {
          ...
          const [tagMatch, ...tagMatches] = file.tags.filter(
            ({ alias, id }) => {
          ...
          const tagToAdjust =
            tagMatch || makeTag(args.tag, args.id || makeId());
          ...
          tagToAdjust.nodes = nodes.filter(
            (n: string) => !args.remove.includes(n),
          );
      ...
      result: [
        ...
        ({ adjustTag, getTags }, cbk: any) => {
          ...
          return cbk(null, {
            tags: parse(getTags).tags.filter(
              (n: { nodes: string | any[] }) => !!n.nodes && !!n.nodes.length,
            ),
          });
        },
      ],
