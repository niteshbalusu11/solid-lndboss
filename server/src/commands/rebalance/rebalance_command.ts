...
const rebalanceCommand = async ({
  args,
  lnd,
  logger,
}): Promise<{ result: any }> => {
  const avoidArray = !!args.avoid ? args.avoid.filter((n: string) => !!n) : [];
  const inFiltersArray = !!args.in_filters
    ? args.in_filters.filter((n: string) => !!n)
    : [];
  const outFiltersArray = !!args.out_filters
    ? args.out_filters.filter((n: string) => !!n)
    : [];
