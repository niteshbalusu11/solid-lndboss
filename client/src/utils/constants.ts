...
export const selectedSavedNode = () => {
  if (
    ...
    localStorage.getItem('SELECTED_SAVED_NODE') !== 'undefined'
  ) {
...
export const removeStyling = (o: any) =>
  JSON.parse(
...
// Convert sats to btc
export const tokensAsBigTokens = (tokens: number) =>
  !!tokens ? (tokens / 1e8).toFixed(8) : 0;

// Server Url
export const serverUrl = import.meta.env.VITE_VARIABLE_NAME_LNDBOSS_SERVER_URL;
